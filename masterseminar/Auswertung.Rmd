...
huxreg(reg_underpricing_1, reg_underpricing_2, reg_underpricing_3)
```
...
As measured with the Cumulative Average Return (CAR)
Day 2 to 31
```{r}
...
As measured by the buy-and-hold-abnormal-return (BHAR)
$$
\text{BHAR} = \frac{1}{N} \sum_{i=1}^N \left( \prod_{t=t_i}^{T_i} (1+R_{it}) -1 \right) - \left( \prod_{t=t_i}^{T_i} (1+ \mathbf{E}[R_{it}]) -1 \right)
$$

where $R_{it}$ is the return on security $i$ in month $t$ adjusted for dividends, $N$ is the number of securities, $T$ is the number of months (12, 36, 60), $t_i$ is the date of the closing price on the first day of trading and $\mathbf{E}[R_{it}]$ is the expected return.
